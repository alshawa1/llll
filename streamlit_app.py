...
st.markdown("""
    ...
    .login-card:hover {
        box-shadow: 0 25px 60px rgba(0, 0, 0, 0.6), 0 0 40px rgba(168, 85, 247, 0.35);
    }
    ...
    .stButton > button {
        background: linear-gradient(135deg, #7c3aed 0%, #9333ea 50%, #4f46e5 100%) !important;
    ...
    .stButton > button:hover {
        ...
        background: linear-gradient(135deg, #8b5cf6 0%, #a855f7 50%, #6366f1 100%) !important;
    }
    .stButton > button:active {
        transform: translateY(0px) !important;
    }
    
    /* ── Header Banner (After Login) ── */
    .header-banner {
        background: linear-gradient(135deg, rgba(30, 20, 60, 0.6) 0%, rgba(15, 23, 42, 0.8) 100%);
    ...
    st.markdown(f"""
        <div class="sidebar-logo-card">
            ...
            <div style="margin-top:8px;">
                <span class="header-tag">{'👥 ' if st.session_state.sector == 'افراد' else '🏢 '}{st.session_state.get('sector_display', 'النظام')}</span>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
if st.session_state.sector == "افراد" or st.session_state.sector == "افرad":
    import افراد_app
