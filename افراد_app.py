...
def run_afrad_app():
    ...
    MODULES = {
        ...
        "coverage_targets": {
            "name": "🎯 التغطية ومستهدف التغطية",
            "desc": "برنامج مخصص لحساب تغطية العملاء الفراداوين ومقارنة الأداء بمستهدف التغطية (نسبة مئوية أو عدد عملاء) وحساب الفجوة والتغطية اليومية والتجميعية وتفاصيل العملاء المغطين وغير المغطين.",
            "id": 12,
            "files": [
                {"key": "portfolio", "label": "ملف المحفظة الأساسية (.xlsx) - مطلوب", "required": True},
                {"key": "payments", "label": "ملف السدادات (.xlsx) - اختياري", "required": False}
            ]
        }
    }
    ...
    def read_excel_calamine(file_path: str) -> pl.DataFrame:
        try:
            ...
            for i, h in enumerate(data[0]):
                ...
                if not h_str or h_str in ("None", "nan"):
                    h_str = f"Column_{i}"
                if h_str in seen:
                    seen[h_str] += 1
            ...
            n_cols = len(headers)
            str_records = [r + [""] * (n_cols - len(r)) if len(r) < n_cols else r[:n_cols] for r in str_records]
            ...
            st.warning(f"⚠️ حدث خطأ أثناء قراءة الملف: {e}")
            return pl.DataFrame()
    ...
    # ════════════════════════════════════════════════════════════════════
        ...
        # ─── واجهة التغطية ومستهدف التغطية ───
        cov_params = {}
        if selected_key == "coverage_targets" and uploaded_files.get("portfolio"):
            st.markdown("#### 🎯 إعدادات التغطية ومستهدف التغطية:")

            c_d1, c_d2 = st.columns(2)
            with c_d1:
                st.markdown("##### 📅 فترة التغطية (Coverage Date):")
                cov_s = st.date_input("من تاريخ التغطية:", datetime.today() - timedelta(days=7), key="cov_t_start")
                cov_e = st.date_input("إلى تاريخ التغطية:", datetime.today(), key="cov_t_end")
                cov_params["coverage_start_date"] = cov_s.strftime("%Y-%m-%d")
                cov_params["coverage_end_date"] = cov_e.strftime("%Y-%m-%d")
            with c_d2:
                st.markdown("##### 💳 فترة السدادات (Payment Data Period):")
                pay_s = st.date_input("من تاريخ السداد:", datetime.today() - timedelta(days=7), key="pay_t_start")
                pay_e = st.date_input("إلى تاريخ السداد:", datetime.today(), key="pay_t_end")
                cov_params["payment_start_date"] = pay_s.strftime("%Y-%m-%d")
                cov_params["payment_end_date"] = pay_e.strftime("%Y-%m-%d")

            st.markdown("<div class='purple-divider'></div>", unsafe_allow_html=True)
            t_col1, t_col2, t_col3 = st.columns(3)
            with t_col1:
                tgt_type_lbl = st.radio(
                    "نوع مستهدف التغطية:",
                    ["نسبة مئوية (Target %)", "عدد عملاء ثابت (Target Customers)"],
                    index=0,
                    horizontal=True
                )
                cov_params["target_type"] = "percent" if "نسبة" in tgt_type_lbl else "count"
            with t_col2:
                if cov_params["target_type"] == "percent":
                    cov_params["target_value"] = st.number_input("مستهدف التغطية (%):", min_value=1.0, max_value=100.0, value=80.0, step=5.0)
                else:
                    cov_params["target_value"] = st.number_input("مستهدف التغطية (عدد عملاء):", min_value=1, value=5000, step=500)
            with t_col3:
                cov_params["collection_target_value"] = st.number_input("💰 مستهدف التحصيل المالي (ريال):", min_value=0.0, value=0.0, step=5000.0, help="اتركه 0 إذا لم ترغب بفرض مستهدف مالي")

            st.markdown("##### 🟢🟡🔴 مستويات تقييم الأداء (Thresholds):")
            th_c1, th_c2 = st.columns(2)
            with th_c1:
                cov_params["threshold_green"] = st.number_input("🟢 نسبة تحكم المستهدف للون الأخضر (%):", min_value=50.0, max_value=200.0, value=100.0, step=5.0)
            with th_c2:
                cov_params["threshold_yellow"] = st.number_input("🟡 نسبة تحكم المستهدف للون الأصفر (%):", min_value=10.0, max_value=100.0, value=80.0, step=5.0)
        ...
        if selected_key == "electronic" and not elec_params:
            ready_to_run = False
        if selected_key == "coverage_targets" and not cov_params:
            ready_to_run = False
        ...
        if st.button("🚀 تشغيل التحليل والمعالجة", disabled=not ready_to_run, use_container_width=True):
            ...
            try:
                with st.spinner("⏳ جاري قراءة وتجهيز الملفات..."):
                    ...
                    for k, vr in results.items():
                        if not vr.is_valid:
                            ...
                            st.stop()
                        # Show warnings softly without stopping
                ...
                with st.spinner("⚙️ جاري معالجة البيانات وتطبيق القواعد الحسابية..."):
                    ...
                    if task_id == 1:
                        ...
                        if ops_params.get("is_targeting_report"):
                            ...
                            # تشغيل تقرير الاستهداف (التصنيف التفصيلي)
                            ...
                            # ── تشغيل module9 لحساب ملخص التغطية والتحصيل (perf_summary) ──
                            ops_r = OperationsReportModule().run(
                                portfolio,
                                payments=pmt_df if pmt_df is not None and len(pmt_df) > 0 else None,
                                report_mode=ops_params.get("report_mode", "daily"),
                                target_date=ops_params.get("target_date"),
                                start_date=ops_params.get("start_date"),
                                end_date=ops_params.get("end_date"),
                            ...
                            r["perf_summary"] = ops_r.get("perf_summary")
                            r["report_period"] = ops_r.get("report_period", "")
                        ...
                        from modules.module12_coverage_targets import CoverageTargetsModule
                        pmt_df = dfs.get("payments")
                        cov_mod = CoverageTargetsModule()
                        cov_r = cov_mod.run(
                            portfolio,
                            payments_in=pmt_df,
                            coverage_start_date=cov_params.get("coverage_start_date"),
                            coverage_end_date=cov_params.get("coverage_end_date"),
                            payment_start_date=cov_params.get("payment_start_date"),
                            payment_end_date=cov_params.get("payment_end_date"),
                            target_type=cov_params.get("target_type", "percent"),
                            target_value=cov_params.get("target_value", 80.0),
                            collection_target_value=cov_params.get("collection_target_value", 0.0),
                            threshold_green=cov_params.get("threshold_green", 100.0),
                            threshold_yellow=cov_params.get("threshold_yellow", 80.0),
                        )
                        if "error" in cov_r:
                            st.error(cov_r["error"])
                            st.stop()
                        stats.update(cov_r["stats"])
                        r = cov_r
                ...
                # جدول ملخص الأداء — يظهر لكلا وضعي التقرير (استهداف / عمليات)
                if task_id == 9 and 'r' in locals():
                    st.markdown("---")
                    period_lbl = r.get("report_period", "")
                    st.markdown(f"### 📊 ملخص أداء التغطية والتحصيل")
                    st.caption(f"مفلتر على: **{period_lbl}** — التغطية من المحفظة · التحصيل من ملف السدادات")

                    perf_df = r.get("perf_summary")
                    if perf_df is not None and not perf_df.is_empty():
                        import pandas as pd
                        perf_pd = perf_df.to_pandas()

                        col_cfg = {
                            "المشرف": st.column_config.TextColumn("👤 المشرف", width="medium"),
                            "المحصلين التابعين له": st.column_config.TextColumn("👥 المحصلين", width="large"),
                            "التغطية": st.column_config.NumberColumn("✅ التغطية (عدد)", format="%d"),
                            "مستهدف التغطية": st.column_config.NumberColumn("🎯 مستهدف التغطية", format="%d"),
                            "نسبة التغطية %": st.column_config.ProgressColumn(
                                "📊 التغطية %", format="%.1f%%", min_value=0, max_value=100
                            ),
                            "التحصيل": st.column_config.NumberColumn("💰 التحصيل (ريال)", format="%.2f ﷼"),
                            "مستهدف التحصيل": st.column_config.NumberColumn("🏦 مستهدف التحصيل", format="%.2f ﷼"),
                            "نسبة التحصيل %": st.column_config.ProgressColumn(
                                "📈 التحصيل %", format="%.1f%%", min_value=0, max_value=100
                            ),
                        }
                        display_cols = ["المشرف", "المحصلين التابعين له",
                                        "التغطية", "مستهدف التغطية", "نسبة التغطية %",
                                        "التحصيل", "مستهدف التحصيل", "نسبة التحصيل %"]
                        show_cols = [c for c in display_cols if c in perf_pd.columns]

                        # الجدول الكامل (مشرفين + محصلين + إجمالي)
                        st.dataframe(
                            perf_pd[show_cols],
                        ...
                        # ملخص المشرفين فقط
                        if "النوع" in perf_pd.columns:
                            sup_rows = perf_pd[perf_pd["النوع"] == "مشرف"]
                            if not sup_rows.empty:
                                st.markdown("#### 📋 ملخص المشرفين فقط")
                                st.dataframe(
                                    sup_rows[show_cols],
                                    use_container_width=True,
                                    hide_index=True,
                    ...
                    # البيانات التفصيلية جوه expander (لو كان تقرير استهداف)
                    if ops_params.get("is_targeting_report") and "data" in r:
                        with st.expander("📄 عرض البيانات التفصيلية للعملاء (تقرير الاستهداف)", expanded=False):
                ...
                # ─── 🎯 شاشة نتائج برنامج التغطية ومستهدف التغطية (Module 12) ───
                if task_id == 12 and 'cov_r' in locals():
                    cov_res = cov_r
                    st.markdown("---")
                    st.markdown("### 🎯 نتائج برنامج التغطية ومستهدف التغطية (سيستم الأفراد)")
                    st.caption(f"فترة التغطية: **{cov_res.get('coverage_period_str')}** | فترة السدادات: **{cov_res.get('payment_period_str')}**")

                    if cov_res.get("period_warning"):
                        st.warning(cov_res["period_warning"])

                    # 📥 زر تحميل تقرير التغطية المخصص
                    from modules.module12_coverage_targets import CoverageTargetsModule
                    excel_cov_bytes = CoverageTargetsModule().export_excel(cov_res)
                    st.download_button(
                        label="📥 تحميل تقرير التغطية الشامل (Excel Multi-Sheet)",
                        data=excel_cov_bytes,
                        file_name=f"تقرير_التغطية_ومستهدف_التغطية_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
                    ...
                    # ─── بطاقات KPI الرئيسية (Main Dashboard Cards) ───
                    kpi_c1, kpi_c2, kpi_c3, kpi_c4, kpi_c5, kpi_c6 = st.columns(6)
                    kpi_c1.metric("👥 إجمالي العملاء", f"{cov_res['total_eligible']:,}")
                    kpi_c2.metric("🎯 مستهدف التغطية", f"{cov_res['target_customers']:,}")
                    kpi_c3.metric("✅ العملاء المغطاة", f"{cov_res['total_covered']:,}")
                    kpi_c4.metric("📊 نسبة التغطية", f"{cov_res['coverage_rate_pct']:.2f}%")
                    kpi_c5.metric("🏆 تحقيق المستهدف", f"{cov_res['target_achievement_pct']:.2f}%")
                    kpi_c6.metric("⚠️ المتبقي للمستهدف", f"{cov_res['gap_customers']:,}")

                    st.markdown("<div class='purple-divider'></div>", unsafe_allow_html=True)

                    # ─── التبويبات التفصيلية ───
                    tab_daily, tab_coll, tab_sup, tab_port, tab_cov_custs, tab_uncov_custs = st.tabs([
                        "📈 التغطية اليومية والتجميعية",
                        "👤 تغطية المحصلين",
                        "👥 تغطية المشرفين",
                        "💼 تغطية المحافظ",
                        "✅ العملاء المغطاة",
                    ...
                    # 1. التغطية اليومية
                    with tab_daily:
                        st.markdown("#### 📅 التغطية اليومية والتجميعية الفريدة (Cumulative Unique Coverage)")
                        df_d = cov_res.get("daily_coverage", pd.DataFrame())
                        if not df_d.empty:
                            st.dataframe(df_d, use_container_width=True, hide_index=True)
                            
                            # الرسم البياني للتغطية التجميعية مقارنة بالمستهدف
                            import plotly.graph_objects as go
                            fig_cov = go.Figure()
                            fig_cov.add_trace(go.Scatter(
                                x=df_d["التاريخ"], y=df_d["التغطية التجميعية (فريد)"],
                                mode="lines+markers", name="التغطية التجميعية الفعلية (عملاء فراداوين)",
                                line=dict(color="#10b981", width=3)
                            ))
                            fig_cov.add_trace(go.Scatter(
                                x=df_d["التاريخ"], y=df_d["المستهدف الكلي"],
                                mode="lines", name="مستهدف التغطية الكلي",
                                line=dict(color="#ef4444", width=2, dash="dash")
                            ))
                            fig_cov.update_layout(
                                title="منحنى التغطية التجميعية اليومية الفريدة مقابل المستهدف",
                                xaxis_title="التاريخ", yaxis_title="عدد العملاء",
                                template="plotly_dark", height=400
                            )
                            st.plotly_chart(fig_cov, use_container_width=True)
                        else:
                            st.info("لا توجد بيانات تغطية يومية متوفرة في هذه الفترة.")

                    # 2. تغطية المحصلين
                    with tab_coll:
                        st.markdown("#### 👤 تغطية المحصلين وتقييم الأداء (Collector Coverage Ranking)")
                        df_c = cov_res.get("collector_coverage", pd.DataFrame())
                        if not df_c.empty:
                            st.dataframe(
                                df_c,
                                use_container_width=True,
                                hide_index=True,
                                column_config={
                                    "تحقيق المستهدف %": st.column_config.ProgressColumn(
                                        "🏆 تحقيق المستهدف %", format="%.2f%%", min_value=0, max_value=100
                                    ),
                                    "التغطية %": st.column_config.NumberColumn("📊 التغطية %", format="%.2f%%")
                                }
                            )
                        else:
                            st.info("لا توجد بيانات محصلين متوفرة.")

                    # 3. تغطية المشرفين
                    with tab_sup:
                        st.markdown("#### 👥 تغطية المشرفين وتتبع المستهدفات (Supervisor Coverage)")
                        df_s = cov_res.get("supervisor_coverage", pd.DataFrame())
                        if not df_s.empty:
                            st.dataframe(
                                df_s,
                                use_container_width=True,
                            ...
                            st.info("لا توجد بيانات مشرفين متوفرة.")

                    # 4. تغطية المحافظ
                    with tab_port:
                        st.markdown("#### 💼 تغطية المحافظ الخاصة بسيستم الأفراد (Portfolio Coverage)")
                        df_p = cov_res.get("portfolio_coverage", pd.DataFrame())
                        if not df_p.empty:
                            st.dataframe(df_p, use_container_width=True, hide_index=True)
                        else:
                            st.info("لا توجد بيانات محافظ متوفرة.")

                    # 5. العملاء المغطاة
                    with tab_cov_custs:
                        st.markdown("#### ✅ قائمة العملاء المغطاة (Covered Unique Customers)")
                        df_cov_c = cov_res.get("covered_customers", pd.DataFrame())
                        if not df_cov_c.empty:
                            st.caption(f"إجمالي العملاء المغطين فراداوياً: **{len(df_cov_c):,}** عميل")
                            st.dataframe(df_cov_c, use_container_width=True, hide_index=True)
                        else:
                            st.info("لا يوجد عملاء مغطين خلال هذه الفترة.")

                    # 6. العملاء غير المغطاة
                    with tab_uncov_custs:
                        st.markdown("#### ❌ قائمة العملاء غير المغطاة (Not Covered Unique Customers)")
                        df_uncov_c = cov_res.get("not_covered_customers", pd.DataFrame())
                        if not df_uncov_c.empty:
                            st.caption(f"إجمالي العملاء غير المغطين فراداوياً: **{len(df_uncov_c):,}** عميل")
                            st.dataframe(df_uncov_c, use_container_width=True, hide_index=True)
                        else:
                            st.info("جميع العملاء تم تغطيتهم بنجاح!")
