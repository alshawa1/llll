...
# ═══════════════════════════════════════════════════════════════════
    ...
    if has_period:
        ...
        if coverage_df.empty:
            ...
            period_label = sel_date or (f"{start_date} → {end_date}") or sel_month
            st.markdown(f"### نتائج الفترة: **{period_label}**")
            ...
            st.markdown("---")
            st.markdown("#### 📋 جدول التغطية والتحصيل:")

            fmt_dict = {
                'إجمالي العملاء': '{:,}', 'العملاء المغطين': '{:,}',
                'نسبة التغطية %': '{:.1f}%',
                'توصل': '{:,}', 'عدم توصل': '{:,}', 'لا يرد': '{:,}', 'مغلق': '{:,}',
                'إجمالي المتابعة اليومية': '{:,}', 'إجمالي التحصيل': '{:,.2f}',
                'نسبة التوصل %': '{:.1f}%',
                'مستهدف التغطية': '{:,}', 'نسبة تحقيق التغطية %': '{:.1f}%',
                'مستهدف التحصيل': '{:,.2f}', 'نسبة تحقيق التحصيل %': '{:.1f}%',
            }
            active_fmt = {k: v for k, v in fmt_dict.items() if k in coverage_df.columns}
            st.dataframe(coverage_df.style.format(active_fmt), use_container_width=True)
            ...
            st.download_button("📥 تحميل تقرير التغطية Excel", data=excel_bytes,
