...
class ExcelExporter:
    ...
    def export_coverage_report(coverage_df: pd.DataFrame, selected_date: str, company_name: str = 'مهاره لتحصيل الديون') -> bytes:
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            title_df = pd.DataFrame({'تاريخ التقرير': [selected_date], 'الشركة': [company_name], 'نوع التقرير': ['تقرير التغطية والتوصل اليومي']})
            title_df.to_excel(writer, sheet_name='تقرير التغطية', index=False, startrow=0)
            coverage_df.to_excel(writer, sheet_name='تقرير التغطية', index=False, startrow=3)
